...
// ==========================================
// CORREÇÃO DEFINITIVA DO SCROLL (MENU LATERAL)
// Intercepta os links âncora para não esconder o título atrás do header
// ==========================================
document.addEventListener("DOMContentLoaded", () => {
    // Seleciona todos os links da barra lateral que começam com "#"
    const linksSidebar = document.querySelectorAll('.universal-sidebar a[href^="#"]');
    ...
    linksSidebar.forEach(link => {
        link.addEventListener("click", function (e) {
            ...
            if (targetElement) {
                // Aqui definimos o desconto do tamanho do cabeçalho de vidro (100px)
                const offset = 100;

                // Calcula a posição exata do elemento na tela
                const elementPosition = targetElement.getBoundingClientRect().top;

                // Subtrai o tamanho do cabeçalho da posição final
                const offsetPosition = elementPosition + window.scrollY - offset;

                // Faz a rolagem suave até o ponto exato
                window.scrollTo({
                    top: offsetPosition,
                    behavior: "smooth"
                });
